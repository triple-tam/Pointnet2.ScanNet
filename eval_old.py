...
def forward(args, model, coords, feats):
    ...
    outputs = pred.max(3)[1] # T: taking max probability value in the 3rd dim

    return outputs

def filter_points(coords, preds, targets, weights):
    '''T: filtering points based on the uniqueness of their hash codes (based on the vertex values)'''
...
def compute_acc(coords, preds, targets, weights):
    ...
    # T: is voxcaliacc not the same as simply total_correct_vox / total_seen_vox ? so aka voxcaliacc = voxacc ?
    # T: what seems to be missing is mAcc = np.sum((total_correct_class / total_seen_class) * mask) / np.sum(mask)

    pointacc_per_class = np.zeros(NUM_CLASSES)
    voxacc_per_class = np.zeros(NUM_CLASSES)
    for l in seen_classes:
        if l == 0: continue
        pointacc_per_class[l] = total_correct_class[l]/float(total_seen_class[l] + 1e-8)
        voxacc_per_class[l] = total_correct_class_vox[l]/float(total_seen_class_vox[l] + 1e-8)
...
def compute_miou(coords, preds, targets, weights):
    ...
    for l in seen_classes:
        if l == 0: continue
        target_label = np.arange(targets.shape[0])[targets==l] # T: getting the indexes of where gt = label l
        pred_label = np.arange(preds.shape[0])[preds==l]
        num_intersection_label = np.intersect1d(pred_label, target_label).shape[0] # #correct preds for label l
        num_union_label = np.union1d(pred_label, target_label).shape[0] # T: union of preds and gt's for label l
...
def eval_one_batch(args, model, data):
    ...
    assert acc_mask.all() == miou_mask.all() # T: not a check to whether they're all the same, but rather that they have matching True values
...
def eval_wholescene(args, model, dataloader):
    ...
    for load_idx, data in enumerate(dataloader):
        ...
        pointacc_per_class_array[load_idx] = pointacc_per_class[1:] # T: so many inconsistencies with indexing merp
...
def evaluate(args):
    ...
    avg_pointacc_per_class = np.sum(pointacc_per_class_array * masks, axis=0) / np.sum(masks, axis=0) # T: avg with masks, cannot use np.mean(); result is vector from all the batches
    ...
    avg_pointacc_per_class2 = np.array([a/b if b!=0 else 0 for a,b in zip(tmp_num, tmp_den)])
    mean_pointacc_per_class = np.sum(avg_pointacc_per_class2) / np.sum(avg_pointacc_per_class2 > 0)
    ...
    avg_voxacc_per_class2 = np.array([a/b if b!=0 else 0 for a,b in zip(tmp_num, tmp_den)])
    mean_voxacc_per_class = np.sum(avg_voxacc_per_class2) / np.sum(avg_voxacc_per_class2 > 0)
    ...
    avg_pointmiou_per_class2 = np.array([a/b if b!=0 else 0 for a,b in zip(tmp_num, tmp_den)])
    mean_pointmiou_per_class = np.sum(avg_pointmiou_per_class2) / np.sum(avg_pointmiou_per_class2 > 0)
    ...
    avg_voxmiou_per_class2 = np.array([a/b if b!=0 else 0 for a,b in zip(tmp_num, tmp_den)])
    mean_voxmiou_per_class = np.sum(avg_voxmiou_per_class2) / np.sum(avg_voxmiou_per_class2 > 0)

    # report
    print()
    print("Point accuracy: {}".format(avg_pointacc))
    print("Point accuracy per class: {}".format(mean_pointacc_per_class)) #T: mask of seen classes shouldn't be necc, bc there shouldn't exist 0 ??
